...
public class TextListAdapter extends RecyclerView.Adapter<TextListAdapter.TextHolder> {
    ...
    public TextHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new TextHolder(mContent.getLayoutInflater().inflate(R.layout.cat_item_list, parent, false));

    }

    @Override
    public void onBindViewHolder(final TextHolder holder,final int position) {
        holder.hend.setText(position+1+"");//设置头部的文字//时间展示
        holder.name.setText(Util.getName(position));//设置名称//miaomi名称
